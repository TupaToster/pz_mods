...
function F350.extractCond(player)
	local vehicle = player:getVehicle()
	if (vehicle and string.find( vehicle:getScriptName(), "90fordF350ambulance" )) then
		for i, partId in ipairs ({
			"Engine", "EngineDoor",
			"Windshield", "WindowFrontLeft", "WindowFrontRight", "WindowRearRight", "WindshieldRear",
			"DoorFrontLeft", "DoorFrontRight", "DoorRearRight", "DoorRear",
...
function F350.flushCond(player)
	local vanillaExit = ISExitVehicle["perform"];
        ...
        ISExitVehicle["perform"] = function(self)
            local vehicle = self.vehicle
				if (vehicle and string.find( vehicle:getScriptName(), "90fordF350ambulance" )) then
					for i, partId in ipairs ({
						"SeatRearLeft", "SeatRearRight", "Stretcher", "F350InsideFrontStorage", "F350InsideLeftStorage", "F350InsideRightTopStorage", "F350InsideRightBottomStorage",
						})
					do
...
function F350.activeArmor(player)
    local vehicle = player:getVehicle()
    	if (vehicle and string.find( vehicle:getScriptName(), "90fordF350ambulance" )) then
		...
		--
			...
			for partId, armorPartId in pairs({
				["DoorFrontLeft"] = "F350WindowFrontLeftArmor",
				["DoorFrontRight"] = "F350WindowFrontRightArmor",
				["DoorRearRight"] = "F350WindowRearRightArmor",
			}) do
		...
		for i, freezeState in ipairs ({"SeatRearLeft", "SeatRearRight", "Stretcher", "F350InsideFrontStorage", "F350InsideLeftStorage", "F350InsideRightTopStorage", "F350InsideRightBottomStorage",})
				do
...
Events.OnEnterVehicle.Add(F350.extractCond);
Events.OnPlayerUpdate.Add(F350.activeArmor);
Events.OnGameStart.Add(F350.flushCond);