...
function M998.flushCond(player)
	local vanillaExit = ISExitVehicle["perform"];
        ...
        ISExitVehicle["perform"] = function(self)
            local vehicle = self.vehicle
				if (vehicle and string.find( vehicle:getScriptName(), "92amgeneralM998" )) then
					for i, partId in ipairs ({
						"M998Roofrack", "M998SpareTire", "GasTank", 
						})
					do
...
function M998.activeArmor(player)
    local vehicle = player:getVehicle()
		...
		for i, freezeState in ipairs ({"M998Roofrack", "M998SpareTire", "GasTank"})
				do
...
Events.OnGameStart.Add(M998.flushCond);
